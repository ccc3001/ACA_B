...
void print_fullmatrix(pcfullmatrix f)
{
  int i, j;
...
void fill_fullmatrix(pfullmatrix f, int d, const double *nodes_x, const double *nodes_y,
...
#ifdef USE_BLAS
void add_fullmatrix(pcfullmatrix f, pfullmatrix g, double a)
{
  int numentries;
...
#ifdef USE_BLAS
void addeval_fullmatrix(pcfullmatrix f, const double *v, double *w)
{
  assert(f != NULL);
  assert(f->rows >= 0);
  assert(f->cols >= 0);
...
void eval_fullmatrix(pcfullmatrix f, const double *v, double *w)
{
...
#ifdef USE_BLAS
void mul_fullmatrix(pcfullmatrix f, pcfullmatrix g, pfullmatrix fg)
{
  assert(f != NULL);
  assert(g != NULL);
  assert(fg != NULL);
...
new_submatrix(pcfullmatrix f, int row_start, int col_start, int rows, int cols)
{
  ...
  assert(row_start + rows <= f->rows);
  assert(col_start + cols <= f->cols);

  s = new_fullmatrix(rows, cols);

  for (j = 0; j < cols; j++)
  {
    for (i = 0; i < rows; i++)
    {
      s->e[j * rows + i] = f->e[(col_start + j) * f->rows + (row_start + i)];
    }
  }
